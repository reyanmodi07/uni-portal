...
async function startServer() {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("leave_group_member", async (data) => {
        const { groupId, userId } = data;
        const groups = await db.getGroups();
        const group = groups[groupId];
    ...
    socket.on("join_group_by_code", async (data, callback) => {
        const { inviteCode, userId } = data;
        console.log(`Attempting to join group with code: ${inviteCode} for user: ${userId}`);
        const groups = await db.getGroups();
        const group = Object.values(groups).find((g: any) => g.inviteCode === inviteCode);
