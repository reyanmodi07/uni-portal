...
const Dashboard: React.FC<DashboardProps> = ({ subjects, tasks, timetable, userName, setView }) => {
  ...
  // Load timer settings, mood, and streak
  useEffect(() => {
    ...
    if (lastVisit === today) {
        ...
        if (lastVisit === yesterday.toDateString()) {
            ...
            // Reset to 1 if missed a day or first visit
            setStreak(1);
                   ...
                   >
                     ...
                     <div className="flex justify-center items-center gap-6">
                        <motion.button 
                           whileTap={{ scale: 0.9 }}
                           onClick={toggleTimer} 
                           className={`w-20 h-20 rounded-[2rem] flex items-center justify-center text-white transition-all shadow-xl hover:shadow-2xl hover:scale-105 active:scale-95 ${isTimerActive ? 'bg-amber-500 shadow-amber-500/20' : 'bg-zinc-900 shadow-zinc-900/20'}`}
                        >
