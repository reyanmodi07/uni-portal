...
const Groups: React.FC<GroupsProps> = ({ messages, setMessages, groups = [], setGroups, userName = 'Student' }) => {
  ...
  const myGroups = groups.filter(g => g.members.includes(userName) || g.members.includes('me'));
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
        ...
        groups.forEach(g => {
            if (g.members.includes(userName) || g.members.includes('me')) {
    ...
    socket.on('group_deleted', (deletedGroupId: string) => {
        if (setGroups) {
  ...
  const handleJoinGroup = (e: React.FormEvent) => {
    ...
    // Use server-side validation to find group by code
    socketRef.current?.emit('join_group_by_code', { 
        ...
        if (response.success) {
            ...
            // Update local state immediately with the group returned by server
            setGroups(prev => {
            ...
            alert(response.error || "Invalid Invite Code");
        }
    });
  };
    ...
    if (window.confirm(`Are you sure you want to leave ${activeGroup.name}?`)) {
       setGroups(prevGroups => prevGroups.map(g => {
           if (g.id === activeGroup.id) {
               return { ...g, members: g.members.filter(m => m !== userName && m !== 'me') };
           }
           return g;
       }));
       socketRef.current?.emit('leave_group', activeGroup.id);
       socketRef.current?.emit('leave_group_member', { groupId: activeGroup.id, userId: userName });
  ...
  const handleDeleteGroup = () => {
     ...
     if (window.confirm(`Are you sure you want to delete ${activeGroup.name}? This cannot be undone.`)) {
         ...
         socketRef.current?.emit('delete_group', activeGroup.id);
         setIsSettingsModalOpen(false);
         setActiveGroupId(''); // Force re-selection
     }
  };
  ...
  const handleVote = (msgId: string, optionId: string) => {
    ...
    setMessages(prev => prev.map(msg => {
        ...
        const newVotes = { ...msg.pollData.votes, [userName]: optionId };
        ...
        <div className="min-w-[260px]">
            ...
            <div className="space-y-2">
                {msg.pollData.options.map(opt => {
                    ...
                    const isVoted = msg.pollData?.votes[userName] === opt.id || msg.pollData?.votes['me'] === opt.id;
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-zinc-900 relative">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-slate-50/50 dark:bg-black/20">
             {activeMessages.map((msg) => {
               const isMyMessage = msg.senderId === userName || msg.senderId === 'me';
               return (
               <div key={msg.id} className={`flex ${isMyMessage ? 'justify-end' : 'justify-start'} group relative`}>
                  ...
                  <div className={`flex flex-col ${isMyMessage ? 'items-end' : 'items-start'} max-w-[85%] sm:max-w-[70%]`}>
                      {!isMyMessage && (
                      ...
                      <div className={`p-4 rounded-2xl shadow-sm relative text-sm group/bubble ${
                        msg.type === 'poll' 
                          ...
                          <button 
                            ...
                            className={`absolute -right-8 top-1/2 -translate-y-1/2 p-1.5 rounded-full text-zinc-400 hover:text-amber-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 opacity-0 group-hover/bubble:opacity-100 transition-all ${isMyMessage ? '-left-8 right-auto' : '-right-8'}`}
                    ...
                    <div className="max-h-64 overflow-y-auto custom-scrollbar space-y-3 pr-2">
                        {activeGroup.members.map((memberId, idx) => (
                             <div key={idx} className="flex items-center gap-3 p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-100 dark:border-zinc-700">
                                 ...
                                 <div className="flex-1 min-w-0">
                                     <p className="font-bold text-zinc-900 dark:text-white text-sm truncate">{memberId === userName || memberId === 'me' ? 'You' : memberId}</p>
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white dark:bg-zinc-900 w-full max-w-sm p-8 rounded-[2rem] shadow-2xl border border-zinc-200 dark:border-zinc-800">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="pt-4 space-y-3 border-t border-zinc-100 dark:border-zinc-800">
                             ...
                             {(activeGroup.createdBy === userName || activeGroup.createdBy === 'me') && (
                                 <button 
